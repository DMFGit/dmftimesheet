-- Fix critical security vulnerability: Role-Based Access Control
-- Replace overly permissive employee update policy with granular ones

-- Drop the existing overly permissive update policy
DROP POLICY IF EXISTS "Users can update own record" ON public."Employees";

-- Create granular policies that prevent role escalation
...
WITH CHECK (
  ...
  AND active = OLD.active  -- Prevent deactivating themselves
);

-- Admin-only policy for role management and employee administration
...
-- Fix any unlinked employee records by adding constraints
-- First, let's add a constraint to prevent future unlinked records
ALTER TABLE public."Employees" 
ADD CONSTRAINT employees_must_have_user_id 
CHECK (user_id IS NOT NULL OR (auth.jwt() ->> 'email'::text) = 'dina@dmfengineering.com'::text);
