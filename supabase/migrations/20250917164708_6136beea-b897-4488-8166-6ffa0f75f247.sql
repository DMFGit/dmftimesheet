-- Fix critical security vulnerability: Role-Based Access Control
-- Clean up and replace existing policies with secure ones

-- Drop all existing update policies to start fresh
DROP POLICY IF EXISTS "Users can update own record" ON public."Employees";
DROP POLICY IF EXISTS "Users can update own profile data" ON public."Employees";

-- Create secure policy that prevents role/status escalation
-- This allows users to update only their name and email
CREATE POLICY "Users can update safe profile fields only" 
ON public."Employees" 
FOR UPDATE 
USING (auth.uid() = user_id)
WITH CHECK (auth.uid() = user_id);

-- Add trigger to prevent role/status changes by non-admins
CREATE OR REPLACE FUNCTION public.prevent_role_escalation()
RETURNS TRIGGER AS $$
BEGIN
  -- Check if non-admin is trying to change sensitive fields
  IF auth.uid() IS NOT NULL AND (auth.jwt() ->> 'email'::text) != 'dina@dmfengineering.com'::text THEN
    -- Prevent role changes
    IF OLD.role IS DISTINCT FROM NEW.role THEN
      RAISE EXCEPTION 'Permission denied: Cannot modify role';
    END IF;
    
    -- Prevent status changes  
    IF OLD.active IS DISTINCT FROM NEW.active THEN
      RAISE EXCEPTION 'Permission denied: Cannot modify active status';
    END IF;
    
    -- Prevent billing rate changes
    IF OLD."Default Billing Rate" IS DISTINCT FROM NEW."Default Billing Rate" THEN
      RAISE EXCEPTION 'Permission denied: Cannot modify billing rate';
    END IF;
  END IF;
  
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Create trigger to enforce role security
DROP TRIGGER IF EXISTS prevent_role_escalation_trigger ON public."Employees";
CREATE TRIGGER prevent_role_escalation_trigger
  BEFORE UPDATE ON public."Employees"
  FOR EACH ROW
  EXECUTE FUNCTION public.prevent_role_escalation();

-- Create security audit log table if it doesn't exist
CREATE TABLE IF NOT EXISTS public.security_audit_log (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id uuid REFERENCES auth.users(id),
  action text NOT NULL,
  table_name text NOT NULL,
  record_id uuid,
  old_values jsonb,
  new_values jsonb,
  ip_address inet,
  user_agent text,
  created_at timestamp with time zone NOT NULL DEFAULT now()
);

-- Enable RLS on audit log
ALTER TABLE public.security_audit_log ENABLE ROW LEVEL SECURITY;

-- Drop existing audit log policy if it exists
DROP POLICY IF EXISTS "Admin can view audit logs" ON public.security_audit_log;

-- Only admins can view audit logs
CREATE POLICY "Admin can view audit logs" 
ON public.security_audit_log 
FOR SELECT 
USING ((auth.jwt() ->> 'email'::text) = 'dina@dmfengineering.com'::text);

-- Create function to log sensitive changes
CREATE OR REPLACE FUNCTION public.log_employee_changes()
RETURNS TRIGGER AS $$
BEGIN
  -- Log role changes (only admins can do this)
  IF OLD.role IS DISTINCT FROM NEW.role THEN
    INSERT INTO public.security_audit_log (
      user_id, action, table_name, record_id, old_values, new_values
    ) VALUES (
      auth.uid(),
      'role_change',
      'Employees',
      NEW.id,
      jsonb_build_object('role', OLD.role),
      jsonb_build_object('role', NEW.role)
    );
  END IF;
  
  -- Log status changes
  IF OLD.active IS DISTINCT FROM NEW.active THEN
    INSERT INTO public.security_audit_log (
      user_id, action, table_name, record_id, old_values, new_values
    ) VALUES (
      auth.uid(),
      'status_change',
      'Employees',
      NEW.id,
      jsonb_build_object('active', OLD.active),
      jsonb_build_object('active', NEW.active)
    );
  END IF;
  
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Create audit trigger
DROP TRIGGER IF EXISTS employee_security_audit ON public."Employees";
CREATE TRIGGER employee_security_audit
  AFTER UPDATE ON public."Employees"
  FOR EACH ROW
  EXECUTE FUNCTION public.log_employee_changes();