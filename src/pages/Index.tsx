...
const INITIAL_QUICK_ENTRY_FORM: QuickEntryForm = {
  selectedDate: "",
  entryMode: "manual",
  projectId: "",
  taskId: "",
...
const Index = () => {
  ...
  const [quickEntryForm, setQuickEntryForm] = useState<QuickEntryForm>(INITIAL_QUICK_ENTRY_FORM);
  ...
  const recentEntries = useMemo(() => {
    ...
    sortedEntries.forEach((entry) => {
      if (uniqueEntries.size >= MAX_RECENT_ENTRIES) return;
  ...
  const approvedHours = weekTimeEntries
  ...
  const billablePercentage = weeklyHours > 0 ? (billableHours / TARGET_BILLABLE_HOURS) * 100 : 0;
  ...
  const openQuickEntry = useCallback((date: string) => {
    setEditingEntry(null);
    setQuickEntryForm({
      ...INITIAL_QUICK_ENTRY_FORM,
      selectedDate: date,
    });
  ...
  const handleDeleteEntry = useCallback(
    ...
    [deleteTimeEntry, toast],
  );

  const loadRecentEntry = useCallback((recentEntry: RecentEntry) => {
  ...
  const updateQuickEntryForm = useCallback((field: keyof QuickEntryForm, value: string | number) => {
    setQuickEntryForm((prev) => {
  ...
  const findBudgetItem = useCallback(
    (projectId: string, taskId: string, subtaskId: string) => {
      const subtaskNumber = subtaskId ? parseFloat(subtaskId) : null;

      return budgetItems.find(
        (item) =>
          item.project_number === parseInt(projectId) &&
          item.task_number === parseInt(taskId) &&
  ...
  const handleQuickSubmit = useCallback(async () => {
    ...
    try {
      const budgetItem = findBudgetItem(quickEntryForm.projectId, quickEntryForm.taskId, quickEntryForm.subtaskId);

      if (!budgetItem) {
        const errorMsg = quickEntryForm.subtaskId
          ? "Could not find WBS code for selected subtask"
          : "Could not find WBS code for selected task";
      ...
      setEditingEntry(null);
      setQuickEntryForm(INITIAL_QUICK_ENTRY_FORM);
    } catch (error) {
      console.error("Error saving time entry:", error);
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to save time entry. Please try again.",
        variant: "destructive",
      });
    }
  }, [quickEntryForm, editingEntry, findBudgetItem, updateTimeEntry, addTimeEntry, toast]);
  ...
  }, [draftEntries, employee, currentWeekStart, weeklyHours, toast]);

  const getEditableEntry = useCallback(
    (dayKey: string, projectKey: string) => {
      return weekTimeEntries.find(
        (entry) =>
  ...
  const hasRejectedEntry = useCallback(
    (dayKey: string, projectKey: string) => {
      return weekTimeEntries.some(
        (entry) => entry.entry_date === dayKey && entry.wbs_code === projectKey && entry.status === "rejected",
      );
    },
    [weekTimeEntries],
  );
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm font-medium text-primary-foreground/80">
                    {billableHours}h of {TARGET_BILLABLE_HOURS}h target
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-foreground">
                    {Math.round((weeklyHours / DAYS_IN_WEEK) * 10) / 10}h
                  </p>
                </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              {viewMode === "weekly" ? (
                <>
                  <Button variant="outline" size="sm" onClick={() => navigateWeek("prev")} aria-label="Previous week">
                  ...
                  <Button variant="outline" size="sm" onClick={() => navigateWeek("next")} aria-label="Next week">
                    Next Week
              ...
              {authLoading || timeEntriesLoading ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse">
                    <thead>
                      <tr>
                        <th className="text-left p-3 border-b border-border font-semibold">Project</th>
                        ...
                        return (
                          <tr key={projectKey} className="hover:bg-muted/50">
                                      ...
                                      return budgetItem ? (
                                        <div className="text-sm space-y-1">
                                          {budgetItem.task_description && (
                                            <div>
                                              <strong>Task:</strong> {budgetItem.task_description}
                                            </div>
                                          )}
                                          {budgetItem.subtask_description && (
                                            <div>
                                              <strong>Subtask:</strong> {budgetItem.subtask_description}
                                            </div>
                                          )}
                                          {budgetItem.task_unit && (
                                            <div>
                                              <strong>Unit:</strong> {budgetItem.task_unit}
                                            </div>
                                          )}
                                        </div>
                                      ) : null;
                            ...
                            {weekDays.map((day, index) => {
                              ...
                              const editableEntry = getEditableEntry(dayKey, projectKey);
                              const hasRejected = hasRejectedEntry(dayKey, projectKey);
                                    ...
                                    <div className="flex flex-col items-center gap-1">
                                      <div className="flex items-center gap-1">
                                        <Tooltip>
                                          <TooltipTrigger asChild>
                                            <Badge
                                              variant={hours >= FULL_WORK_DAY ? "default" : "secondary"}
                                        ...
                                        {hasRejected && <AlertTriangle className="h-3 w-3 text-destructive" />}
                                      </div>
                                      ...
                                      {editableEntry && (
                                        <div className="flex gap-1">
                                          <Button
                                            ...
                                            onClick={() => openEditEntry(editableEntry)}
                                          ...
                                          <Button
                                            ...
                                            onClick={() => handleDeleteEntry(editableEntry.id)}
                    ...
                    <tfoot>
                      <tr className="bg-muted/30">
                        <td className="p-3 border-t border-border font-semibold">Daily Totals</td>
                        {weekDays.map((day, index) => {
                          const dayKey = format(day, "yyyy-MM-dd");
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      <div className="space-y-2">
                        <div className="flex items-center gap-4">
                          ...
                          {weeklyHours >= TARGET_BILLABLE_HOURS && (
                      ...
                      <div className="flex gap-2">
                        {hasUnsubmittedEntries && (
                          <Button onClick={handleSubmitTimesheet} className="bg-primary hover:bg-primary/90" size="lg">
          ...
          <Card className="shadow-lg">
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Button size="sm" onClick={() => openQuickEntry(selectedDate)}>
                  <Plus className="h-4 w-4 mr-2" />
                  Add Entry
                </Button>
              </div>
            ...
            <CardContent>
              {authLoading || timeEntriesLoading ? (
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              {entry.status === "rejected" && <AlertTriangle className="h-3 w-3 text-destructive" />}
                            </div>
                              ...
                              <div className="space-y-1">
                                ...
                                <div className="text-sm space-y-1">
                                  {entry.taskDescription && (
                                    <div>
                                      <strong>Task:</strong> {entry.taskDescription}
                                    </div>
                                  )}
                                  {entry.subtaskDescription && (
                                    <div>
                                      <strong>Subtask:</strong> {entry.subtaskDescription}
                                    </div>
                                  )}
                                </div>
                              </div>
                        ...
                        <div className="text-right">
                          <div className="text-2xl font-bold text-primary mb-1">{entry.hours}h</div>
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Click Add Entry to create your first time entry for{" "}
                    {format(parseDateSafe(selectedDate), "MMM d, yyyy")}
                  </p>
                </div>
              )}
            </CardContent>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className="w-full justify-start text-left font-normal">
                      ...
                      {quickEntryForm.selectedDate ? (
                        format(parseDateSafe(quickEntryForm.selectedDate), "PPP")
                      ) : (
                        ...
                        <div className="grid gap-2 max-h-60 overflow-y-auto">
                          {recentEntries.map((recent, index) => (
                            <Button
                              ...
                              type="button"
                              variant={
                                quickEntryForm.projectId === recent.projectId &&
                                quickEntryForm.taskId === recent.taskId &&
                                quickEntryForm.subtaskId === recent.subtaskId
                                  ? "default"
                                  : "outline"
                              }
                      ...
                      >
                        ...
                        <SelectContent className="bg-popover z-50">
                          {getSubtasksByTask(quickEntryForm.projectId, quickEntryForm.taskId).map((subtask) => {
                            const parentTask = getTasksByProject(parseInt(quickEntryForm.projectId)).find(
                              (t) => t.task_number.toString() === quickEntryForm.taskId,
                            );
                            return (
                              <SelectItem key={subtask.id} value={subtask.id}>
                                {subtask.number} - {subtask.description}
                                {parentTask?.task_unit && (
                                  <span className="text-muted-foreground ml-2">({parentTask.task_unit})</span>
                                )}
                                {subtask.wbs_code && (
                                  <span className="text-xs text-muted-foreground ml-2">[{subtask.wbs_code}]</span>
                                )}
                              </SelectItem>
                            );
                          })}
